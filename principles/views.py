...
def add_auth(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            x = form.save()
            add_log(request.user.pk,x,"added an authority")
...
def authority_update(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            x = form.save()
            log_change(request.user.pk,x,"authority has been added")
            messages.success(request, "authority has been added")
            return redirect('principles:authority_list')
        else:
...
def authority_delete(request, pk):
    ...
    if request.method == "POST":
        log_deletion(request.user.pk,auth,"deleted authority")
        auth.delete()
...
def principle_update(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            x = form.save()
            log_change(request.user.pk,x,"changed a Principle")
            messages.success(request, "Principle has been updated")
...
def principle_delete(request, pk):
    ...
    if request.method == "POST":
        log_deletion(request.user.pk,prin,"deleted a principle")
        prin.delete()
...
def add_principle(request):
    ...
    if request.method == 'POST':
        ...
        if p_form.is_valid():
            ...
            add_log(request.user.pk,x,"added a principle")
