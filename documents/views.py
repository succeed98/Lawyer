...
def add_document(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # print(archive)
            x = doc.save()
            add_log(request.user.pk,x,"added a document")
...
def update_document(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            log_change(request.user.pk,x,"changed a document")
...
def record_update(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            log_change(request.user.pk,x,"changed a record document")
...
def record_delete(request, pk):
    ...
    if request.method == "POST":
        log_deletion(request.user.pk,rec,"deleted record document")
        rec.delete()
        messages.success(
...
def request_doc(request, pk):
    ...
    if doc:
        ...
        log_change(request.user.pk,x,"made a request")
...
def document_delete_view(request, pk):
    ...
    if request.method == "POST":
        log_deletion(request.user.pk,doc,"deleted document")
        doc.delete()
...
def approve_request(request, pk):
    ...
    log_change(request.user.pk,x,"approved document")
    messages.success(
    ...
    # print(archive)
    msg2 = "Dear {} , your reqeust for {} has been approved.".format(
        rec.requested_by, rec.document)
    ...
    if request.method == "POST":
        ...
        if file_form.is_valid():
            for f in files:
                ...
                add_log(request.user.pk,new_files,"added a file")
                print(f.name)
                print(new_files)
...
def delete_file(request, pk):
    ...
    if file:
        log_deletion(request.user.pk,file,"deleted file")
        file.delete()
