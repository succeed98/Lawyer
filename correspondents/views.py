...
def add_corrs(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            x = form.save()
            add_log(request.user.pk,x,"added a correspondent")
...
def update_corrs(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.instance.lawyer = request.user
            x = form.save()
            add_change(request.user.pk,x,"changed a correspondent")
...
def delete_corrs(request, pk):
    ...
    if request.method == "POST":
        add_deletion(request.user.pk,object,"deleted a correspondent")
        object.delete()
...
def edit_cors(request,pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.instance.lawyer = request.user
            x = form.save()
            log_change(request.user.pk,x,"changed a correspondent")
            messages.success(request, "Correspondent has been updated")
            return HttpResponseRedirect(reverse('correspondents:detail', args=[object.pk]))
        else:
            messages.error(request, "Correspondent could not be updated")
        ...
        form = CorrespondentForm(instance=object)

    return render(request, 'correspondents/update.html', {'form': form,'object':object})
