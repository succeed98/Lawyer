...
def add_book(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            add_log(request.user.pk,x,"added a book")
...
def book_update(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.instance.added_by = get_lawyer(request.user)
            x = form.save()
            log_change(request.user.pk,x,"changed a book")
            messages.success(request, "Book has been updated")
...
def delete_book(request, pk):
    ...
    if request.method == "POST":
        log_deletion(request.user.pk,object,"deleted a book")
        object.delete()
        messages.success(request, "Book Has Been Added")
        return redirect('resources:book_list')
...
def request_book(request, pk):
    ...
    if not book.requested:
        ...
        today = timezone.now()
        history = BookHistory.objects.create(
            book=book, lawyer=lawyer, date_requested=today)
...
def approve_book(request, pk):
    ...
    if hist:
        ...
        book.save()
        x = hist.save()
        log_change(request.user.pk,x,"approved a book")
...
def return_book(request, pk):
    ...
    if hist:
        ...
        book.save()
        x = hist.save()
        log_change(request.user.pk,x,"returned a book")
